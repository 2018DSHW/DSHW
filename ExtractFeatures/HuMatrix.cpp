...
void huMatrix()
{
	...
	ofstream outputFile("HuMatrix.txt", ios::out);
	double HuMatrix[7] = { 0.0 };
	...
	while (!inputFile.eof())
	{
		...
		//计算不变矩
		HuMatrix[0] = u20 + u02;
		HuMatrix[1] = t1 * t1 + 4 * u11*u11;
		HuMatrix[2] = t2 * t2 + t3 * t3;
		HuMatrix[3] = t4 * t4 + t5 * t5;
		HuMatrix[4] = t2 * t4*(t4*t4 - 3 * t5*t5) + t3 * t5*(3 * t4*t4 - t5 * t5);
		HuMatrix[5] = t1 * (t4*t4 - t5 * t5) + 4 * u11*t4*t5;
		HuMatrix[6] = t3 * t3*(t4*t4 - 3 * t5*t5) - t2 * t5*(3 * t4*t4 - t5 * t5);

		//outputFile<<lineStr<<'(';
		for (int i = 0; i < 7; i++)
		{
			outputFile << HuMatrix[i] << ' ';
		}
		outputFile << endl;
